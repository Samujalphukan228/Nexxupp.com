...
const AboutPage = () => {
    ...
    const handleContinue = () => {
        setShowPopup(false);
        window.location.href = "/pricing"; // redirect when Continue is clicked
    };
    ...
    return (
        <div className="min-h-screen">
            <div className="max-w-[1400px] mx-auto py-12 sm:py-20 px-4 sm:px-8 space-y-20 sm:space-y-28 lg:space-y-32">
                ...
                {/* About Company */}
                <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10 lg:gap-16 items-center">
                    <div className="relative h-[220px] sm:h-[300px] lg:h-[460px] w-full overflow-hidden bg-gray-50 rounded-sm order-2 lg:order-1">
                        <Image
                            src="/mid.png"
                            alt="Company"
                            fill
                    ...
                    <div className="space-y-4 sm:space-y-5 order-1 lg:order-2">
                        <div className="space-y-2">
                            <h2 className="text-[1.5rem] sm:text-[2.5rem] font-light tracking-tight text-gray-900 leading-[1.1] sm:leading-[1.2]">
                                About NexxUpp
                            </h2>
                            <div className="w-14 h-[1px] bg-gray-300"></div>
                        </div>
                        <p className="text-[13px] sm:text-[16px] leading-[1.6] sm:leading-[1.8] text-gray-500 font-light">
                        ...
                        <div className="pt-2 sm:pt-3">
                            <div className="inline-flex items-center gap-2 text-gray-700 font-medium text-[11px] sm:text-[12px] group cursor-pointer">
                                ...
                                <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 group-hover:translate-x-1 transition-transform duration-500 ease-out" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                        {workSteps.map(({ title, desc, number }) => (
                            <div key={title} className="space-y-3 pb-5 border-b border-gray-200">
                                <div className="text-[11px] sm:text-[12px] font-light text-gray-400 tracking-wider">
                                    {number}
                                </div>
                                <h3 className="text-[1.2rem] sm:text-[1.5rem] font-light text-gray-900 leading-[1.2]">
                                    {title}
                                </h3>
                                <p className="text-[12px] sm:text-[14px] leading-[1.6] text-gray-500 font-light">
                                    {desc}
                                </p>
                            </div>
                        ))}
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        {solutions.map(({ title, desc }, index) => (
                            <div
                                key={title}
                                className="relative p-5 sm:p-6 border border-gray-200 rounded-sm overflow-hidden"
                            >
                                ...
                                <div className="relative z-10 space-y-2 sm:space-y-3">
                                    ...
                                    <p className="text-[12px] sm:text-[14px] leading-[1.6] text-gray-500 font-light max-w-full sm:max-w-md">
                                        {desc}
                                    </p>
                                </div>
                            </div>
